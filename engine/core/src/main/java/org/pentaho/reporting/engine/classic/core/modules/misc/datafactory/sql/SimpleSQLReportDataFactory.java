...
public class SimpleSQLReportDataFactory extends AbstractDataFactory implements IReportCancelEvent {
  ...
  private static void setFetchSize( String driverName, Statement statement ) throws SQLException {
    if ( driverName.contains( "mysql" ) || driverName.contains( "mariadb" ) ) {
      ...
      statement.setFetchSize( Integer.parseInt(
                        globalConfig.getConfigProperty( //$NON-NLS-1$
                                ResultSetTableModelFactory.POSTGRES_FETCH_SIZE ) ) );
    }
  }
  ...
  public ResultSet performQuery( Statement statement, final String translatedQuery, final String[] preparedParameterNames )
          throws SQLException {
    final ResultSet res;
    String fetchSizeStr = globalConfig.getConfigProperty(
            ResultSetTableModelFactory.POSTGRES_FETCH_SIZE );
    int fetchSize = 0; // Default fetch size
    if ( fetchSizeStr != null && !fetchSizeStr.trim().isEmpty() ) {
      try {
